...
\tls{Get rid of iterative counterfactual prediction alg, do one step. That aligns with many genomics setups.}

\begin{abstract}
    % \ndfg{This is a great start. I do think we should position our result more generally, not just as a generalization of DiD (which it is of course). We are creating parallel trends in general non-abelian settings, so that's probably how I would mention it first, and then focus on DID). Also, we'll need to address the recent paper by Mueller if we ONLY go for DID generalization. Happy to discuss}
    Predicting counterfactual dynamics -- how systems would evolve when subjected to alternative conditions -- is a fundamental challenge in machine learning and causal inference. Many approaches to tackle this challenge leverage a key assumption: identical populations subject to differing initial conditions exhibit dynamics that evolve in \textit{parallel}. A recurring obstacle with this conceptual framework, however, is the fact that many modern data objects of interest (distributions, point-clouds, etc.) live in non-vector spaces where abelian operations like subtraction are not available. To address this, we introduce a general notion of \textit{parallelism of temporal trends} in Riemannian spaces based on \textit{parallel transport} of tangent trends along geodesics. We call this principle \textit{Riemannian Parallel Trends}, and it ensures that trends observed for one population can be transported to a different baseline population in a consistent manner via geodesic parallel transport, enabling rigorous counterfactual comparison even when initial states differ. We then instantiate this framework on the Wasserstein space of probability measures $(\mathcal{P}_2(\mathbb{R}^d), W_2)$, yielding a distributional, high-dimensional analogue of parallel trends. In the Euclidean special case, where geodesics are straight lines and parallel transport is the identity, \textit{Riemannian Parallel Trends} reduces to the classical Difference-in-Differences (DiD) parallel trends assumption and recovers the usual DiD estimand. Beyond this conceptual framework, we also propose a tractable algorithm for approximating parallel transport along Wasserstein geodesics, and develop an unbalanced counterpart on $(\mathfrak{M}_+(\mathbb{R}^d), \mathrm{HK})$ (Hellinger–Kantorovich / Wasserstein–Fisher–Rao) to accommodate mass variation. Finally, we demonstrate the utility of these methods for counterfactual trajectory reconstruction on genomics datasets with unbalanced dynamics and on economics datasets where mass is conserved.
\end{abstract}
...
\subsection{Optimal Transport} Define $\mathcal{P}_{2}(\man)$ to be the set of probability measures with finite $2^{\text{nd}}$ moment over a Riemannian manifold $\man$ with metric tensor $g$. In this section we will take $M =\mathbb{R}^d$, but in later sections we will explore other choices of $M$. The $2$-Wasserstein distance between two probability measures $\mu, \nu \in \mathcal{P}_{2}(\mathbb{R}^d)$ is defined by
\begin{equation}
    W_2(\mu, \nu) = \inf_{\gamma \in \Gamma_{\mu, \nu}}\left(\int_{\mathbb{R}^d} \|x-y\|_2^2\,\gamma(dx, dy)\right)^{1/2} \label{eq: 2 wasserstein dist}
\end{equation}
...
This theorem guarantees that if the source measure has a density, then the optimal transport coupling can be written as the source measure $\mu$ and $\nabla \varphi_\#\mu$, the pushforward of the source under some convex function. The function $\nabla\varphi$ is often referred to as the \textit{Brenier map}. This key theorem will be central to our construction of Riemannian structure on the space of probability measures. 

\subsection{Dynamic Formulation and Unbalanced Transport}

The optimal transport problem described above -- one which looks for an optimal coupling of source and target measures -- is often referred to as the \textit{static} formulation of optimal transport. An equivalent perspective comes from fluid mechanics, which arrives at the same metric structure through a different formulation. In particular, let $(v_t)_{t \geq 0}$ be a time dependent family of vector fields over $\mathbb{R}^d$, and consider the ODE $\dot X_t = v_t(X_t)$. Let $\mu_t$ denote the law of $X_t$, where $X_0 \sim \mu_0$ and $X_t$ evolves according to the ODE described previously. Then, the dynamics of $\mu_t$ obey the so-called \textit{continuity equation},
\begin{equation}
    \partial_t\mu_t + \nabla \cdot (\mu_t v_t) = 0 \label{eq: balanced continuity eqn}
\end{equation}
in the weak (or distributional) sense. 
\begin{restatable}[Weak solutions to \Cref{eq: balanced continuity eqn}, \citet{santambrogio2015optimal}]{defn}{}
    We say that a family of pairs $(\mu_t, v_t)$ with $v_t \in L_1(\rho_t; \mathbb{R}^d)$ and $\int_0^1\|v_t\|_{L_1(\mu_t)}dt < \infty$ solves the continuity equation on $(0, T)$ in the distributional sense if for any bounded and Lipschitz test function $\varphi \in C^1_c((0, T)\times \mathbb{R}^d)$ we have
    \[\int_0^T\int_{\mathbb{R}^d} \partial_t\varphi \, d\mu_tdt + \int_0^T\int_{\mathbb{R}^d} \nabla\varphi \cdot v_t \, d\mu_tdt = 0.\]
\end{restatable}
...
\begin{restatable}[Benamou-Brenier, \citet{chewi2024statisticaloptimaltransport}]{thm}{}
    Let $\mu_0, \mu_1 \in \mathcal{P}_{2}(\mathbb{R}^d)$ be absolutely continuous with respect to Lebesgue. Then 
    \begin{equation*}
        W_2^2(\mu_0, \mu_1) = \inf \left\{\int_{0}^1\|v_t\|^2_{L_2(\mu_t)} dt \, \Bigg| \, (\mu_t, v_t)_{t \in [0,1]} \text{ solves \cref{eq: balanced continuity eqn}}\right\}.
    \end{equation*}
...
In classical balanced transport, it is only possible to transport a measure $\mu$ to a measure $\nu$ if they have the same total measure. In many applications, it can be useful to consider situations where total mass can change over time. The dynamical formulation stated above, which considers minimum cost transport between probability measures, can be augmented to allow for \textit{unbalanced} transport. To do this, we can create a modified continuity equation that allows for the creation and destruction of mass,
\begin{equation}
...
\begin{restatable}[Hellinger-Kantorovich Distance, \cite{liero2018optimal, liero2016optimal, kondratyev2016newoptimaltransportdistance, chizat2018interpolating}]{defn}{}
    Let $\mathfrak{M}_+(\mathbb{R}^d)$ denote the space of non-negative measures on $\mathbb{R}^d$. For any absolutely continuous $\mu_0, \mu_1 \in \mathfrak{M}_+(\mathbb{R}^d)$ we define the Hellinger-Kantorovich distance to be
    \begin{equation}
        \text{HK}^2(\mu_0, \mu_1) = \inf\left\{\int_0^1 \left(\|v_t\|_{L_2(\mu_t)}^2 + 4|\beta_t|_{L_2(\mu_t)}^2\right)dt \, \Bigg| \, (\mu_t, v_t,  \beta_t)_{t\in [0,1]} \text{ solves \cref{eq: unbalanced continuity eqn}}\right\}.
    \end{equation}
\end{restatable}

\subsection{Riemannian Structure of Wasserstein and Hellinger-Kantorovich}

\noindent \textbf{The Tangent Space.} The dynamic formulations of balanced and unbalanced optimal transport will be the key ingredients in establishing the Riemannian structure of $(\mathcal{P}_2(\mathbb{R}^d), W_2)$ and $(\mathfrak{M}_+(\mathbb{R}^d), \text{HK})$. In particular, we will construct the tangent space from space of solutions to the continuity equation. But seeing as there are an infinite number of solutions, (as adding a divergence free field does not change the marginal behavior of $\mu_t$) we need to establish a selection principle. To do so, we will define a notion of a \say{derivative} in a general metric space.
...
Now, for a pair of probability measures $\mu, \nu \in \mathcal{P}_{2}(\mathbb{R}^d)$ where $\mu$ admits a density, we will write $T_{\mu \rightarrow \nu}$ for the Brenier map from $\mu$ to $\nu$ and we will write $|\dot \mu|$ for the metric derivative of a curve in $\mathcal{P}_{2}(\mathbb{R}^d)$ with respect to the Wasserstein metric.
...
Note that result coupled with Brenier's theorem indicates that the velocity field that coincides (in an $L_2(\mu_t)$ sense) with the minimal norm solution and the metric derivative of the path is a limit of gradients -- in particular, $(T_{\mu_t \rightarrow \mu_{t+h}} - I_d)(x) = \nabla (\varphi(x) - \frac{1}{2}\|x\|_2^2)$. This key fact gives rise to an intuitive definition of the tangent space and a metric tensor for $(\mathcal{P}_{2}(\mathbb{R}^d), W_2)$ at a measure $\mu$. 

\begin{restatable}[Wasserstein Tangent Space]{defn}{}
    Let $\mu \in \mathcal{P}_2(\mathbb{R}^d)$. We define the tangent space to $\mathcal{P}_2(\mathbb{R}^d)$ at $\mu$ to be 
    \begin{equation}
        T_\mu \mathcal{P}_2(\mathbb{R}^d) = \overline{\left\{\nabla \varphi \, | \, \varphi \in \mathcal{C}_c^{\infty}\right\}}^{L_2(\mu)}
    \end{equation}
    where $\overline{\{\cdot \}}^{L_2(\mu)}$ denotes the $L_2(\mu)$ closure and $\mathcal{C}_c^{\infty}$ denotes the set of compactly supported and smooth maps. We also endow $T_\mu \mathcal{P}_2(\mathbb{R}^d)$ with the metric tensor
    \begin{equation}
        \langle \nabla \varphi_1, \nabla \varphi_2\rangle_{\mu} \triangleq \int \langle\nabla\varphi_1, \nabla\varphi_2 \rangle \,d\mu 
    \end{equation}
...
\begin{restatable}[\citet{clancy2021interpolating}]{prop}{}
    ...
    \begin{equation*}
        (v_t, \beta_t) = \min\left\{\|\tilde v_t\|_{L_2(\mu_t)}^2 + 4|\tilde \beta_t|_{L_2(\mu_t)}^2  \, \Big| \, \partial_t\mu_t + \nabla \cdot (\tilde v_t \mu_t) = 4\tilde \beta_t \mu_t\right\}
    \end{equation*}
...
\begin{restatable}[Hellinger-Kantorovich Tangent Space]{defn}{}
    Let $\mu_t$ be an absolutely continuous measure in $\mathfrak{M}_+(\mathbb{R}^d).$ We define the tangent space to $\posm$ at $\mu$ to be 
    \begin{equation}
...
\begin{restatable}[The fundamental theorem of Riemannian Geometry, \cite{petersen2006riemannian}]{thm}{}
    ...
    \end{enumerate}
\end{restatable}
In the case of $(\mathcal{P}_2(\mathbb{R}^d), W_2)$ and $(\posm, \text{HK})$ we can explicitly construct the covariant derivative, but one needs to manually verify that they are torsion free and metric compatible as neither spaces are finite-dimensional Riemannian Manifolds.

\begin{restatable}[Wasserstein Covariant Derivative, \citet{clancy2021interpolating}]{prop}{} \label{Wasserstein covariant deriv}
    Let $(\mu_t)_{t\geq 0}$ be a curve through $\mathcal{P}_2(\mathbb{R}^d)$ with tangent field $\nabla \varphi_t$ solving \cref{eq: balanced continuity eqn}, and therefore driving the dynamics of $\mu_t$. Also let $v_t$ be another vector field along $\mu_t$ and let $\Pi_{\mu_t}$ be the orthogonal projection onto $T_{\mu_t}\mathcal{P}_2(\mathbb{R}^d)$ in $L_2(\mu_t)$. Then the differential operator $\nabla_{(\nabla\varphi_t)}^{W_2}$ given by
    \begin{equation*}
...
\end{restatable}
This result establishes a closed form PDE describing a differential operator with the desired characteristics of the Levi-Civita connection. We remark that $\Pi_{\mu_t}: L_2(\mu_t; \mathbb{R}^d) \rightarrow T_{\mu_t}\mathcal{P}_2(\mathbb{R}^d)$ is simply the orthogonal projection from the space of $L_2(\mu_t)$ vector fields over $\mathbb{R}^d$ to the closure of $L_2(\mu_t)$ \textit{gradient} fields over $\mathbb{R}^d$. In practice, this operation is simply a (Helmholtz) decomposition $v_t = \nabla \varphi_t + w_t$ where $w_t$ is the divergence-free component of the vector field $v_t$. We also have an analogous covariant derivative for the Hellinger-Kantorovich space.
...
Again, the operator $\Pi_{\mu_t}: L_2(\mu_t; \mathbb{R}^d) \times L_2(\mu_t; \mathbb{R}) \rightarrow T_{\mu_t}\mathfrak{M}_+(\mathbb{R}^d)$ is 
the $L_2(\mu_t)$ orthogonal projection. For a field $(v_t, \beta_t)$, it again is a Helmholtz decomposition of $v_t$ and it leaves $\beta_t$ unchanged. 
\\

\noindent \textbf{The Exponential and Logarithmic Maps.} For $\mathcal{P}_{2}(\man)$, the exponential map is defined to be $\mathbf{exp}_{\mu}(u) \triangleq (\exp(u))_\#\mu$ where $\exp(\cdot)$ is the exponential map of $\man$ and $u \in L_2(\mu)$. In the case of $\man = \mathbb{R}^d$, this operation is trivial -- we have
...
\end{equation}
In the case of Hellinger-Kantorovich, the exponential map and logarithmic maps are more challenging to describe and compute. We defer discussion and description of these maps to the latter portion of \Cref{sec: HK geometry via cone}, after we have introduced more mathematical tools to make things more tractable.

\section{Hellinger-Kantorovich Geometry via Cone Lifting} \label{sec: HK geometry via cone}

While Hellinger-Kantorovich geometry allows for a richer notion of transport, the tradeoff is that it induces more computational challenges. The tangent space, the covariant derivative and the exponential/logarithmic maps are all more challenging to describe and compute than their Wasserstein counterparts. Amazingly, \citet{liero2016optimal} show that Hellinger-Kantorovich geometry can be captured (in a delicate sense) from $W_2$ geometry on an augmented base domain. In particular, this construction entails lifting compactly supported positive measures from the base space $\Omega \subset \subset \mathbb{R}^d$ to probability measures on $\mathfrak{C}_{\Omega}$, the metric cone of the base space. Under a particular set of lifts, it turns out that the Wasserstein geometry on the cone recovers Hellinger-Kantorovich geometry on the original space.
...
We have established the structure of a metric cone, and now we have to connect it back to our base space. Given a measure on $\mathfrak{C}_\Omega$, $\lambda \in \mathcal{P}_2(\mathfrak{C}_{\Omega})$ we can project it to a measure in $\mathfrak{M}_+(\Omega)$ with the map $\mathfrak{B}$ characterized by
...
\begin{restatable}[Correspondence of $(\mathcal{P}(\mathfrak{C}_\Omega), W_2)$ and $(\mathfrak{M}_+(\Omega), \text{HK})$, \citet{liero2016optimal} Theorem 3.6]{thm}{} 
...
To compute the Hellinger-Kantorovich distance and a pair of optimal lifts, we will use an equivalent characterization. Let $\eta_j = \Pi^j_\#\eta$ be the marginals of the joint measure $\eta$ and define the Hellinger-Kantorovich entropy-transport functional as
\begin{equation} \label{eq: let functional}
    E(\eta; \mu_0, \mu_1) = \int_{\Omega}F\left(\frac{d\eta_0}{d\mu_0}\right)d\mu_0 + \int_{\Omega}F\left(\frac{d\eta_1}{d\mu_1}\right)d\mu_1 + \int_{\Omega \times \Omega}c(\|x_0 - x_1\|_2)d\eta
\end{equation}
where
\begin{equation*}
    c(L) = \begin{cases}
...
\begin{align} \label{eq: let hk equivalence}
    \text{HK}(\mu_0, \mu_1) &= \inf\left\{E(\eta;\mu_0, \mu_1)^{1/2} \, |\, \eta  \in \mathfrak{M}_+(\Omega \times \Omega), \eta_j  \ll \mu_j\right\}.
\end{align}
They also show that the logarithmic entropy transport functional gives us a way to compute optimal lifts in the sense of \cref{eq: cone wasserstein and hk equivalence}. In particular, assume that $\eta \in \mathfrak{M}_+(\Omega \times \Omega)$ is a minimizer of $E(\cdot\,; \mu_0, \mu_1)$ and for the marginals $\eta_i$ consider the Lebesgue decomposition $\mu_i = \sigma_i\eta_i + \mu_i^\perp$. Then the transport plan $\gamma_{\eta} \in \mathfrak{M}_+(\mathfrak{C}_{\Omega} \times \mathfrak{C}_{\Omega})$ defined by
...
\begin{restatable}[\citet{liero2016optimal}, Corollary 4.4]{prop}{} \label{cone geodesics project to hk geodesics}
    For any optimal lift $\lambda_0, \lambda_1$ of $\mu_0, \mu_1$, an optimal plan $\gamma \in \mathcal{P}_2(\mathfrak{C}_\Omega \times \mathfrak{C}_\Omega)$ in the sense of $W_\mathfrak{C}$ induces a geodesic path in $(\mathfrak{M}_+(\Omega), \text{HK})$ connecting $\mu_0$ and $\mu_1$ given by
...
\end{restatable}
This result indicates that we can interpolate along a Hellinger-Kantorovich geodesic by lifting the source and target measures to $\mathfrak{C}_\Omega$, solving for the optimal plan, performing Wasserstein interpolation on $\mathfrak{C}_\Omega$, and projecting back via $\mathfrak{B}$.
\\
...
\begin{restatable}[Isometry of Lifting and Projection]{thm}{} \label{isometry}
    ...
    \begin{enumerate}[resume*]
        \item and the projection operator $\mathcal{P}_\lambda$ is the isometric inverse of $\mathcal{L}_{\mathfrak{B}\lambda, \lambda}$ $\lambda$ almost everywhere -- in particular, it is an isometry from $L_2(\lambda; \mathfrak{C}_\Omega)$ equipped with the $\mathcal{P}_2(\mathfrak{C}_\Omega)$ metric tensor and $L_2(\mathfrak{B}\lambda; \Omega) \times L_2(\mathfrak{B}\lambda)$ equipped with the Hellinger-Kantorovich metric tensor. 
    \end{enumerate}
\end{restatable}
We provide the proof of \Cref{isometry} in \Cref{sec: proof of isometry}. This result will be the cornerstone of our method for computing parallel transport on $(\mathfrak{M}_+(\Omega), \text{HK})$, as it tells us that we can isometrically map between HK tangents on $\Omega$ and Wasserstein tangents on $\mathfrak{C}_\Omega$. Since isometries preserve parallel transport, we will compute parallel transport on HK by doing so on the cone and projecting back.

\section{Parallel Transport}

Having laid the groundwork for Wasserstein and Hellinger-Kantorovich geometry, we will now move on to discussing parallel transport in detail. In the first part of this section we will describe the characterization of parallel transport via the covariant derivative. In the latter parts of this section, we will discuss an approximation scheme for parallel transport along well behaved geodesics that uses Jacobi fields to avoid the PDE obtained from the covariant derivative.   
...
\end{restatable}
This characterization now allows us to define parallel transport on $(\mathcal{P}_2(\mathbb{R}^d), W_2)$ and $(\mathfrak{M}_+(\Omega), \text{HK})$ using the covariant derivative. Consider a smooth curve $\mu_t$ through $\mathcal{P}_2(\mathbb{R}^d)$ indexed by $t \in (0, 1)$ with the tangent field $\nabla \varphi_t$ driving its dynamics. 
...
While \Cref{Wasserstein parallel transport pde} and \Cref{HK parallel transport pde} describe parallel transport along any curve, solving these PDEs in practice may be challenging, especially in high-dimensional settings. To this end, we will describe an alternative approximate but tractable approach for computing parallel transport \textit{along geodesics} -- our approach leverages the connection between parallel transport on the base space $\man$ and parallel transport on $\mathcal{P}_{2}(\man)$ for any Riemannian manifold $\man$ (as established by \citet{gigli2012second}) to enable parallel transport on $\mathcal{P}_2(\man)$. We will further leverage the connection between $(\mathcal{P}_2(\mathfrak{C}_\Omega), W_\mathfrak{C})$ and $(\mathfrak{M}_+(\Omega), \text{HK})$ to enable Hellinger-Kantorovich parallel transport. Before we describe said approach, we will introduce \textit{Jacobi fields}, which will be central to our method.

\subsection{Approximate Parallel Transport via Jacobi Fields.} 
...
\begin{restatable}[Fanning Scheme, Proposition B.3 of \citet{louis2018fanning}]{prop}{} \label{fanning approximation}
    Let $\Omega \subseteq \man$ be a compact subset of $\man$ with injectivity radius lower bounded by $\eta$, and let $\gamma_p(s) = \exp_p(su)$ be a geodesic through $(\man, g)$ with $\gamma_p([0, s]) \subset \Omega$. Further denote $\text{PT}_{0 \rightarrow s}^\man(v)$ as the parallel transport of $v \in T_{p}\man$ from $p = \gamma_p(0)$ to $\gamma_p(s)$. Then there exists an $A \geq 0$ such that for all $s < \frac{\eta}{\|u\|_g}$ we have   
    \[\left\|s^{-1}j_{p, u}(0, v)(s) - \text{PT}_{0 \rightarrow s}^\man(v)\right\|_g \leq As^2\|v\|_g. \]
\end{restatable}
...
To approximate $\mathcal{P}_2(\man)$ parallel transport we will use the fanning scheme described in \Cref{fanning approximation} and the connection between $\man$ parallel transport and $\mathcal{P}_2(\man)$ parallel transport along regular curves established by Gigli.

\begin{restatable}[Regular Curves, \citet{gigli2012second}]{defn}{}
    ...
    \[\int_0^1\|v_t\|_{L_2(\mu_t)}^2\,dt < \infty \quad \text{and} \quad \int_0^1\text{L}(v_t)\,dt < \infty\]
    where $\text{L}(v_t)$ denotes the spatial Lipschitz constant of the field $v_t$. 
\end{restatable}

Our following result proposes an approximation scheme for parallel transport along \textit{regular geodesics} between a source $\nu$ and a target $\mu$.
...
\begin{restatable}[One step approximation of $\mathcal{P}_{2}(\man)$ Parallel Transport along regular curves]{lemma}{}\label{one step parallel transport approx result} 
    ...
    we have
    \[\|w_s - \text{PT}_{\nu \rightarrow \nu_s}(v)\|_{L_2(\nu_s)} \leq As^2\|v\|_{L_2(\nu)} + \left(e^{\int_0^1\text{L}(\nabla \psi_r)dr} - 1\right)^2\|v\|_{L_2(\nu)}\left(\int_0^s\text{L}(\nabla\psi_r)dr\right)^2.\]
    Moreover, the regularity of $t \mapsto \nu_t$ guarantees that $w_s$ approximates $\text{PT}_{\nu \rightarrow \nu_s}(v)$, the parallel transport of $v$ along the geodesic $\mathbf{exp}_{\mu}(tu)$, in the sense that
    \[\|w_s - \text{PT}_{\nu \rightarrow \nu_s}(v)\|_{L_2(\nu_s)} \leq Cs^2 \quad \text{for some $C>0$ independent of $s$.}\]
\end{restatable}

We provide the proof of \Cref{one step parallel transport approx result} in \Cref{sec: proof of one step parallel transport approx}. This result allows us to approximate parallel transport of a tangent vector $v$ on $(\mathcal{P}_{2}(\man), W_2)$ along \textit{regular geodesics}, provided that we are able to solve for the Jacobi field $j_{\nu, u}$ on the base space $\man$. If one takes the base space $\man = \Omega \subset \subset \mathbb{R}^d$, solving the Jacobi field is trivial. Moreover, if one assumes that the source and target measures are absolutely continuous, then the flow map of the Wasserstein geodesic between them will be essentially invertible. We leverage this result to construct a procedure described in \Cref{alg: W parallel transport} that achieves $O(n^{-1})$ accuracy (where $n$ is the approximation resolution) detailed in \Cref{full approximation of parallel transport along regular curves}, and we provide the proof in \Cref{sec: proof of full approximation of parallel transport along regular curves}.
...
\begin{restatable}[Approximation of $\mathcal{P}_{2}(\man)$ Parallel Transport along regular curves]{thm}{} \label{full approximation of parallel transport along regular curves}
    Consider the setting of \Cref{one step parallel transport approx result}, where $\nu$ and $\mu$ are connected by a regular geodesic and define the approximate parallel transport map $\widehat{\text{PT}}_{s_1 \rightarrow s_2}$ to be the one described by the procedure of \Cref{one step parallel transport approx result}. Fix some $n\in \mathbb{N}$, let $s = 1/n$ and define the maps 
    \[\widehat{\text{PT}}_k \triangleq \widehat{\text{PT}}_{(k-1)s \rightarrow ks} \quad \text{and} \qquad \text{PT}_k \triangleq \text{PT}_{(k-1)s \rightarrow ks}\]
    and let $v_k = (\text{PT}_k \circ \dots \circ \text{PT}_1)(v)$ and $\hat v_k = (\widehat{\text{PT}}_k \circ \dots \circ \widehat{\text{PT}}_1)(v)$. Then we have
    \[\|\hat v_n - v_n\|_{L_2(\nu)} = O(n^{-1}).\]
\end{restatable}
...
\begin{algorithm}[h]
    \begin{algorithmic}[1]
        ...
        \Require Absolutely continuous source measure $\nu \in \mathcal{P}_{2}(\mathbb{R}^d)$, target measure $\mu \in \mathcal{P}_{2}(\mathbb{R}^d)$, tangent vector $v \in T_{\nu}\mathcal{P}_{2}(\mathbb{R}^d)$, approximation resolution $n$.  
        \State Compute the Brenier map $T_{\nu \rightarrow \mu}$ and stepsize $s = W_2(\nu, \mu)/n.$
        \State Compute the maps $F_i = (1 - \frac{i}{n})I_d + \frac{i}{n}T_{\nu \rightarrow \mu}$ for $i \in \{0, \dots, n\}$.
        \State Let $v_1 = v$.
        \For{$i = 1$ to $n$}
            \State Compute the interpolated measure $\nu_i = (F_{i-1})_\#\nu$.
            ...
            \State Compute $v_{i+1} = \Pi_{\nu_{i+1}}\left(s^{-1}j(0, v_i)(s) \circ S_i^{-1}\right) = \Pi_{\nu_{i + 1}}(v_i \circ S_i^{-1})$. 
        \EndFor
        \State \Return $v_{n+1} \in T_\mu \mathcal{P}_{2}(\mathbb{R}^d)$. 
    \end{algorithmic}
\end{algorithm} 

\subsection{Approximate Hellinger-Kantorovich Parallel Transport.} 
While this result allows one to compute parallel transport along regular geodesics, we will also provide an approach that relaxes these assumptions with further approximation -- we pursue this avenue to enable Hellinger Kantorovich parallel transport, as lifted measures using \Cref{alg: let lift} are necessarily singular on $\mathfrak{C}_{\Omega}$ and do not automatically admit a regular geodesic interpolant. As a result, to compute parallel transport in the Hellinger-Kantorovich setting, we want a method that approximately works along any geodesic. As a step towards this goal, we will use the following result that guarantees that the interior of geodesic paths on $\mathcal{P}_c(\man)$ is necessarily regular.
...
We will leverage this property to construct a weak approximation to parallel transport along the \textit{interior} of any geodesic. Note that Gigli's sense of regularity is needed to ensure that parallel transport even exists, as otherwise the tangent space along a curve need not vary smoothly. We first present an important result that relates HK parallel transport to Wasserstein parallel transport on $\mathfrak{C}$ along the \textit{interior} of optimally lifted geodesics.


\begin{restatable}[Commutativity of $\mathcal{P}$ and $\text{PT}$ on the Interior of Lifted Geodesics]{thm}{} \label{commutator}
    Let $(\mu_t)_{t \in [0, 1]}$ be an absolutely continuous HK geodesic and let $\lambda_0, \lambda_1$ be the optimal lift of the endpoints obtained from \Cref{alg: let lift} where $\mu_i^\perp = 0$, and the map $T$ (\Cref{clancy coupling supported on a map}) is $m_T$-strongly monotone and satisfies
    \[(1 + \rho_t(x)(m_T-1)) > \|T(x) - x\|_2 \|\nabla\rho_t(x)\|_2 \quad  \text{for $\lambda_0$ a.e. $x \in \Omega$, for every $t$}.\]
    Also let $\gamma \in \mathcal{P}_2(\mathfrak{C}_\Omega \times \mathfrak{C}_\Omega)$ be an optimal coupling in the sense of $W_\mathfrak{C}$ and let $\lambda_t(z) = Z(z; \cdot, \cdot)_\#\gamma$. Then for $\varepsilon > 0$, for any $t_0, t_1 \in [\varepsilon, 1-\varepsilon]$ and for any $u \in L_2(\lambda_{t_0}; \mathfrak{C}_\Omega)$ we have 
    \[\Big(\text{PT}^{\text{HK}}_{\mu_{t_0} \rightarrow \mu_{t_1}}\circ \mathcal{P}_{\lambda_{\varepsilon}}\Big)(u) = \left(\mathcal{P}_{\lambda_{t_1}}\circ \text{PT}^{W_\mathfrak{C}}_{\lambda_{t_0} \rightarrow \lambda_{t_1}}\right)(u)\]
    where $\text{PT}^{W_\mathfrak{C}}$ denotes Wasserstein parallel transport on $\mathfrak{C}_\Omega$. 
\end{restatable}
We provide the proof of \Cref{commutator} in \Cref{sec: proof of commutator}. Importantly, this result establishes a concrete connection between Hellinger-Kantorovich parallel transport on $\mathfrak{M}_+(\Omega)$ and Wasserstein parallel transport on $\mathfrak{C}_\Omega$ under regularity conditions. To the best of our knowledge, this is the first result that extends the connection between HK on $\Omega$ and Wasserstein on $\mathfrak{C}_\Omega$ from metric structure to Riemannian structure. We also have this important result regarding the approximation of $\mathcal{P}_2(\man)$ parallel transport on the \textit{interior} of regular geodesics, a regime relevant to our setting as the geodesics between lifted measures need not be regular.

\begin{restatable}[{Approximate $\mathcal{P}_{2}(\man)$ Parallel Transport on the Interior of Lifted Geodesics}]{thm}{}\label{parallel transport approx entire geodesic clipped} 
    Let $\mu, \nu \in \mathcal{P}_{2}(\man)$ be a source and target measure and fix $\varepsilon \in (0, 1/2)$. Consider the following setting:
    \begin{enumerate}
        \item (The geodesic connecting $\mu$ and $\nu$): Let $t \mapsto \nu_t$ be a geodesic connecting $\nu_0 = \nu$ and $\nu_1 = \mu$, fix a tangent field $v \in L_2(\nu_\varepsilon;\man)$ and let $n \in \mathbb{N}$. 
        \item (The estimate): Consider the restriction of $t \mapsto \nu_t$ to $[\varepsilon, 1 - \varepsilon]$, its associated unit-normalized and un-normalized velocity fields $u_t, \nabla \psi_\rho$ respectively, and its flow map $F_{t_1 ,t_2}$ where $\nu_{t_2} = (F_{t_1, t_2})_\#\nu_{t_1}$. Let $s = (1-2\varepsilon)/n$ and set the map $\widehat{\text{PT}}_k: L_2(\nu_{(k-1)s}; M) \rightarrow L_2(\nu_{ks}; \man)$ as
        \[\widehat{\text{PT}}_k = \Big(v \mapsto \Pi_{\nu_{ks}}\left(s^{-1}j_{\nu_{(k-1)s}, u_{ks}}\left(0, v\right)(s) \right) \Big)\]
        with \[\quad j_{\nu_{(k-1)s}, u_{ks}}\left(0, v\right)(s) \triangleq \left(x \mapsto j_{x, u_{ks}(x)}\left(0, v(x)\right)(s)\right) \circ F_{(k-1)s, s}^{-1}\]
        and $\text{PT}_k = \text{PT}_{(k-1)s \rightarrow ks}$. Then for $\hat v_k = (\widehat{\text{PT}}_k \circ \dots \circ \widehat{\text{PT}}_1)(v)$ and $v_k = (\text{PT}_k \circ \dots \text{PT}_1)(v)$ we have
        \[\|\hat v_n - v_n\|_{L_2(\nu)} = O\left(\frac{1}{n} \vee\frac{e^{2K/\varepsilon}}{n^3\varepsilon^2}\right).\]
        Moreover, if $n \gtrsim (e^{K/\varepsilon})/\varepsilon$ then the right hand side of the panel above is $o(1)$.
    \end{enumerate}
\end{restatable}

This result allows us to approximate parallel transport of a tangent vector $v$ on $(\mathcal{P}_{2}(\man), W_2)$ along the interior of any geodesic (it need not be regular) provided that we are able to solve for the Jacobi field $j_{\mu_t, u_t}$ on the base space $\man$. Furthermore, the approximation can be made arbitrarily good by selecting the approximation resolution $n$ sufficiently large relative to the clipping parameter $\varepsilon$. Thus, if we want to approximate parallel transport for $(\mathfrak{M}_+(\Omega), \text{HK})$ then we can solve the logarithmic entropy transport functional problem in \cref{eq: let hk equivalence}, lift our measures to probability measures on $\mathfrak{C}_{\Omega}$ and do parallel transport there. This procedure is described in \Cref{alg: HK parallel transport}.
...
\begin{algorithm}[h]
    \begin{algorithmic}[1]
        ...
        \Require Source measure $\nu \in \mathfrak{M}_+(\Omega)$ and target measure $\mu \in \mathfrak{M}_+(\Omega)$ with $\text{diam}(\Omega) < \pi$, tangent vector $(v, \beta) \in T_{\nu}\mathfrak{M}_+(\Omega)$, approximation resolution $n$, restriction parameter $\varepsilon >0$.  
        \State Compute the optimal lifts $\lambda_{\nu}, \lambda_{\mu} = \text{LiftToCone}(\nu, \mu)$ via \Cref{alg: let lift}
        \State Compute the optimal coupling $\pi^* \in \mathcal{P}_2(\mathfrak{C}_{\Omega} \times \mathfrak{C}_{\Omega})$ and stepsize $s = (1-2\varepsilon)W_\mathfrak{C}(\lambda_\nu, \lambda_\mu)/n.$
        ...
        \For{$i = 1$ to $n$}
            ...
            \State Compute $\left(x \mapsto j_{x, u_i}(0, V_{i-1}^*(x))(s)\right)$ the Jacobi field evaluated at $s$ with initial conditions $0$ \hspace*{\algorithmicindent}and $V^*_{i-1}$, and parallel transport approximation
...
Equipped with an algorithm for approximating Wasserstein and Hellinger-Kantorovich parallel transport, we will now leverage it to construct a procedure for predicting counterfactual dynamics on the space of measures. In this section we will explicitly state the algorithms for the Wasserstein and Hellinger-Kantorovich spaces respectively, along with proofs of convergence of the methods. Thereafter, we will illustrate the utility of these procedures by applying them to the Difference-in-Differences framework in Causal Inference, and to domain adaptation and transfer learning. Finally, we will end with a discussion of further applications of these approaches in economics and sampling.
...
\begin{figure}[t]
  ...
  % % One warped "square-like" manifold
  % \newcommand{\warpsquare}[2]{%
  ...
  %     \node[lab] at (-2.75, 2.0) {\Huge $\mathcal{P}_2(\mathbb{R}^d)$};
  %   \end{scope}
  % }

  % % Manifold backdrop
  % \warpsquare{-2}{0}
    \node[lab] at (-3.75, -0.0) {\Huge $\mathcal{P}_2(\mathbb{R}^d)$};
  % --- Counterfactual dynamics visualization ---
  \begin{scope}[xshift=-3.15cm,yshift=0.1cm]

    % translation for treated pre-trend
    \coordinate (Tshift) at (0.0,-0.70);

    % -----------------------
    % CONTROL curve: nu_t
    % -----------------------
    \coordinate (C0)   at (-1.2, 1.2);
    \coordinate (Ct0)  at (-0.45, 1.35);

    \coordinate (C0c1) at (-0.80, 1.3);

    \coordinate (C1)   at ( 1.75, 1.05);
    \coordinate (C1c1) at (-0.10, 1.40);
    \coordinate (C1c2) at ( 0.55, 1.45);

    \draw[ultra thick, -{Latex[length=2mm]}]
      (C0) .. controls (C0c1) .. (Ct0)
           .. controls (C1c1) and (C1c2) .. (C1);
    \node[lab] at (1.95,1.02) {\Huge $\nu_t$};

    % -----------------------
    % TREATED observed: mu_t
    % -----------------------
    ...
    \draw[ultra thick, -{Latex[length=2mm]}]
      (T0) .. controls (T0c1) .. (Tt0)
          .. controls (1.00, 0.15) and (0.35,-0.45) .. (T1);
    \node[lab] at (1.750,-0.4) {\Huge $\mu_t$};

    % -----------------------
    % TREATED counterfactual: mu_t^*
    % -----------------------
    \coordinate (Tcf)  at ($(C1)  + (Tshift)$);
    \coordinate (T1c1) at ($(C1c1)+ (Tshift)$);
    \coordinate (T1c2) at ($(C1c2)+ (Tshift)$);

    \draw[ultra thick, dashed, -{Latex[length=2mm]}]
      (Tt0) .. controls (T1c1) and (T1c2) .. (Tcf);
    \node[lab] at ($(Tcf)+(0.2,0.00)$) {\Huge $\mu_t^*$};

    % -----------------------
    % t0: tangent on nu_t (red)
    % -----------------------
    \path[
      decoration={
        markings,
        mark=at position 0 with {
          \coordinate (NuBase) at (0,0);
          \draw[ultra thick, draw=black, -{Latex[length=2mm]}] (0,0) -- (1.5,0.1);
          \node[lab, text=black, above] at (0.75,0.15) {\huge $\mathbf{v}_{t_0}$};
        }
      },
      postaction={decorate}
    ] (Ct0) .. controls (C1c1) and (C1c2) .. (C1);


    \node[lab] at ($(Ct0)+(-0.2,0.2)$) {\huge $t_0$};

    % -----------------------
    % t0: define base + direction for mu^* tangent and exp step
    % -----------------------
    \path[
      decoration={
        markings,
        mark=at position 0 with {
          \coordinate (MuBase)   at (0,0);
          \coordinate (MuTanDir) at (0.45,0.04);
          \coordinate (MuExpEnd) at (3.1, -0.1);
        }
      },
      postaction={decorate}
    ] (Tt0) .. controls (T1c1) and (T1c2) .. (Tcf);

    % green dotted "exp map" from t0 -> t1 (underlaid)
    \draw[densely dotted, draw=green!60!black, line width=1.0pt]
      (MuBase)
        .. controls
          ($(MuBase)!1.15!(MuTanDir)$)
          and
          ($(MuExpEnd)+(-0.55,0.0)$)
        .. (MuExpEnd);


    % gray dotted connector between tangent bases at t0
    \path let
      \p1 = (NuBase),
      \p2 = (MuBase),
      \p3 = ($0.5*(NuBase)+0.5*(MuBase) + (-0.01, -0.01)$)
    in
      coordinate (MidBase) at (\x3,\y3);
    \coordinate (Bend) at ($(MidBase)+(0.075,0.1)$);
    \draw[densely dotted, draw=gray, line width=0.8pt]
      (NuBase) .. controls (Bend) and (Bend) .. (MuBase);

    % red tangent arrow for mu^* at t0 (overlaid)
    \draw[ultra thick, draw=green!60!black, -{Latex[length=2mm]}]
      (MuBase) -- ($(MuBase)!3!(MuTanDir)$);
    \node[lab, text=green!60!black, above] at ($(MuBase)!1.6!(MuTanDir)+(0.2,0.05)$)
      {\Large $\widehat{\text{PT}}(\hat{\mathbf{v}}_{t_0})$};
    \node[
  lab,
  draw=black,
  fill=white,
  ...
  text=green!60!black,
  anchor=west
] at ($(MuExpEnd)+(-0.25,-0.32)$) {\huge $\widehat{\mu}_{t_1}^*$};
    % ============================================================
    % Repeat the same pattern for t2 (next step)
    % ============================================================

    % place a new "t1 base" on the counterfactual by interpolating along its post curve
    % (visually: a point later along mu_t^*). Tune 0.55 as desired.
    \coordinate (Tt1) at (MuExpEnd);
    \node[lab] at ($(Tt1)+(-0.05,0.8)$) {\huge $t_1$};

    % place a corresponding control point later along nu_t (for the gray connector)
    \coordinate (Ct1) at ($(Ct0)!0.5!(C1) + (0.0, 0.13)$);

    % tangent on nu_t at t1 (red)
    \path[
      decoration={
        markings,
        mark=at position 0 with {
          \coordinate (NuBaseTwo) at (0,0);
          \draw[ultra thick, draw=black, -{Latex[length=2mm]}] (0,0) -- (1.5,-0.1);
          \node[lab, text=black, above] at (1.0,0.0) {\huge $\mathbf{v}_{t_1}$};
        }
      },
      postaction={decorate}
    ] (Ct1) .. controls ($(Ct1)+(0.35,0.00)$) and ($(Ct1)+(0.75,0.10)$) .. ($(Ct1)+(1.05,-0.05)$);

    % define mu^* base + direction at t1 and exp endpoint t2 (green)
    % We use a short auxiliary curve through Tt1 so the marking has a well-defined tangent frame.
    % Tune controls and endpoints to match the visual direction you want.
    \path[
      decoration={
        markings,
        mark=at position 0 with {
          \coordinate (MuBaseTwo)   at (0,0);
          \coordinate (MuTanDirTwo) at (0.45,-0.05);
          \coordinate (MuExpEndTwo) at (3.0,-1);
        }
      },
      postaction={decorate}
    ] (Tt1) .. controls ($(Tt1)+(0.55,0.10)$) and ($(Tt1)+(1.05,0.15)$) .. ($(Tt1)+(1.45,0.00)$);

    % green dotted exp-map from t1 -> t2 (underlaid)
    \draw[densely dotted, draw=green!60!black, line width=1.0pt]
      (MuBaseTwo)
        .. controls
          ($(MuBaseTwo)!1.15!(MuTanDirTwo)$)
          and
          ($(MuExpEndTwo)+(-0.15,0.05)$)
        .. (MuExpEndTwo);
        \node[
  lab,
  draw=black,
  ...
  text=green!60!black,
  anchor=west
] at ($(MuExpEndTwo)+(-0.35,-0.25)$) {\huge $\widehat{\mu}_{t_2}^*$};

    % gray dotted connector between tangent bases at t1
    \path let
      \p1 = (NuBaseTwo),
      \p2 = (MuBaseTwo),
      \p3 = ($0.5*(NuBaseTwo)+0.5*(MuBaseTwo) + (-0.03, -0.06)$)
    in
      coordinate (MidBaseTwo) at (\x3,\y3);
    \coordinate (BendTwo) at ($(MidBaseTwo)+(0.075,0.1)$);
    \draw[densely dotted, draw=gray, line width=0.8pt]
      (NuBaseTwo) .. controls (BendTwo) and (BendTwo) .. (MuBaseTwo);

    % red tangent arrow for mu^* at t1 (overlaid)
    \draw[ultra thick, draw=green!60!black, -{Latex[length=2mm]}]
      (MuBaseTwo) -- ($(MuBaseTwo)!3!(MuTanDirTwo)$);
    \node[lab, text=green!60!black, above] at ($(MuBaseTwo)!1.6!(MuTanDirTwo)+(0.15,0.02)$)
      {\Large $\widehat{\text{PT}}(\hat{\mathbf{v}}_{t_1})$};
    \fill (Ct0) circle (2pt);
    \fill (Tt0) circle (2pt);
    \fill[green!60!black] (MuExpEnd) circle (1.7pt);
    \fill[green!60!black] (Tt1) circle (2pt);
    \fill[green!60!black] (MuExpEndTwo) circle (1.7pt);
    \fill (Ct1) circle (2pt);
...
\begin{restatable}{asmpt}{} \label{all necessary W assumptions}
    We assume that the measures $(\nu_i)_{i \in \{1, \dots, T\}}$ and $(\mu_i^*)_{i \in [1, \dots, T]}$ are all absolutely continuous, and we require that for all $i \in \{1, \dots , T-1\}$, $\nu_{i}$ and $\nu_{i+1}$ can be connected by a regular $W_2$ geodesic \textit{and} $\nu_i$ and $\mu_i^*$ can be connected by a regular $W_2$ geodesic.
\end{restatable}


\begin{algorithm}[h]
    \begin{algorithmic}[1]
        \caption{Counterfactual trajectory reconstruction on $(\mathcal{P}_{2}(\mathbb{R}^d), W_2)$}
...
\subsection{Hellinger-Kantorovich Counterfactual Dynamics Prediction}



\begin{algorithm}[h]
    \begin{algorithmic}[1] 
        \caption{Counterfactual trajectory reconstruction on $(\mathfrak{M}_+(\Omega), \text{HK})$}
...
\begin{restatable}{asmpt}{} \label{all necessary HK assumptions}
    ...
    \begin{enumerate}
        ...
        \item $\Omega$ is convex, compact and has diameter smaller than $\pi/2$. 
        \item The map $T$ supporting the LET functional coupling $\eta$ is \textit{strongly monotone}, i.e. $\frac{1}{2}(\nabla T + \nabla T^T) \succeq m_T I_d$ for some $m_T > 0$ and
        \[(1 - \rho_t(x)(1 - m_T)) > \|T(x) - x\|_2 \|\nabla\rho_t(x)\|_2 \]
        \textcolor{red}{(this ruins things. Its needed to ensure that the map between cone wasserstein and HK fields is an isometry. I have no idea how to parse it, or whether there even exist measures for which this condition holds)}
    \end{enumerate}
...
\subsection{Riemannian Difference-in-Differences}
...
\subsection{Characterizing the Jacobi equation}
...
\subsubsection{The Covariant Derivative on $\mathfrak{C}_\Omega$}
...
\subsubsection{The Riemann Curvature Tensor on $\mathfrak{C}_\Omega$}
...
\subsubsection{The Jacobi equation}
...
\begin{align*}
    \|w_s - \Pi_{\nu_s}(\tau_0^s(v))\|_{L_2(\nu_s)} &\leq \left\|s^{-1}j_{\nu, u}\left(0, v\right)(s)  - \tau_0^s(v) \right\|_{L_2(\nu_s)} \\
    &= \left(\int \|s^{-1}j_{F_s^{-1}(x), u(F^{-1}_s(x))}\left(0, v(F_s^{-1}(x))\right)(s)  - \text{PT}^\man_{F_s^{-1}(x) \rightarrow x}(v(F_s^{-1}(x))) \|_g^2 \,d\nu_s(x)\right)^{1/2} \\
    &= \left(\int \|s^{-1}j_{y, u(y)}\left(0, v(y)\right)(s)  - \text{PT}^\man_{y \rightarrow F_s(y)}(v(y)) \|_g^2 \,d\nu(y)\right)^{1/2}.
\end{align*}
Now we'll apply \Cref{fanning approximation} to say 
\begin{align*}
    \|s^{-1}j_{y, u(y)}\left(0, v(y)\right)(s)  - \text{PT}^{\man}_{y \rightarrow F_s(y)}(v(y))\|_g \leq As^2\|v(y)\|_g
\end{align*}
for $s$ sufficiently small, which implies 
\begin{align*}
    \|w_s - \Pi_{\nu_s}(\tau_0^s(v))\|_{L_2(\nu_s)} &\leq \left(\int A^2s^4\|v\|_g^2 \,d\nu\right)^{1/2}  = As^2\|v\|_{L_2(\nu)}.
\end{align*}
The second part of the thesis follows from the fact that $t \mapsto \nu_t$ is assumed to be regular and thus admits a velocity field with a spatial Lipschitz constant that integrates to $O(s)$ in the bound of the first term.
\qed{}
...
By \Cref{one step parallel transport approx result} we know that for each $k \in \{1\, \dots , n\}$ we have \[\|\widehat{\text{PT}}_k(w) - \text{PT}_k(w)\|_{L_2(\nu_{ks})} \leq Cs^2\]
for some $C > 0$ independent of $k$. Now let $e_k \triangleq \hat v_k - v_k.$ By the triangle inequality
\begin{align*}
    \|\hat v_k - v_k\|_{L_2(\nu_{sk})}  &= \|\widehat{\text{PT}}_k(\hat v_{k-1}) - \text{PT}_k(v_{k-1})\|_{L_2(\nu_{sk})}\\ &\leq \|\widehat{\text{PT}}_k(\hat v_{k-1}) - \text{PT}_k(\hat v_{k-1})\|_{L_2(\nu_{sk})} + \|\text{PT}_k(\hat v_{k-1}) - \text{PT}_k(v_{k-1})\|_{L_2(\nu_{sk})}.
\end{align*}
Note that the first term is bounded by $Cs^2\|\hat v_{k - 1}\|_{L_2(\nu_{(k-1)s})}$. Since parallel transport is an isometry, we have that the second term can be rewritten to
\[\|\text{PT}_k(\hat v_{k-1}) - \text{PT}_k(v_{k-1})\|_{L_2(\nu_{sk})} = \|\hat v_{k-1} - v_{k-1}\|_{L_2(\nu_{s(k-1)})} = \|e_{k-1}\|_{L_2(\nu_{s(k-1)})}.\]
Putting it together, we have the recurrence relation 
\[\|e_k\|_{L_2(\nu_{sk})} \leq Cs^2\|\hat v_{k - 1}\|_{L_2(\nu_{(k-1)s})} + \|e_{k-1}\|_{L_2(\nu_{s(k-1)})}.\]
By triangle inequality and the isometry of parallel transport, we have $\|\hat v_{k-1}\|_{L_2(\nu_{(k-1)s})} \leq \|v\|_{L_2(\mu)} + \|e_{k-1}\|_{L_2(\nu_{s(k-1)})}$ which implies 
...
\subsection{Proof of \Cref{commutator}} \label{sec: proof of commutator}
First, we need to establish that the radial conditional law of $\lambda_t$ is deterministic for all $t$. This renders $\mathcal{P}_{\lambda_t}$ an isometry (\Cref{isometry} (b)), ensuring that it commutes with parallel transport. \Cref{deterministic radial law}, stated below, indicates that this holds under certain regularity assumptions on the map $T$. \Cref{isometry} then implies that $\mathcal{P}_{\lambda_t}$ is an isometry for those values of $T$. 

\begin{restatable}[Conditional Radial Law on Geodesic Interior]{lemma}{} \label{deterministic radial law}
    Let $(\mu_t)_{t \in [0, 1]}$ be an absolutely continuous HK geodesic and let $\lambda_0, \lambda_1$ be the optimal lift of the endpoints obtained from \Cref{alg: let lift} where $\mu_i^\perp = 0$ and $\eta$ is supported on a map $T$ such that $\frac{1}{2}(\nabla T(x) + \nabla T(x)^T) \succeq m_T I_d$ and
    \[(1 - \rho_t(x)(1 - m_T)) > \|T(x) - x\|_2 \|\nabla\rho_t(x)\|_2 \quad  \text{for $\lambda_0$ a.e. $x \in \Omega$, for every $t$}.\]
    Also let $\gamma \in \mathcal{P}_2(\mathfrak{C}_\Omega \times \mathfrak{C}_\Omega)$ be an optimal coupling in the sense of $W_\mathfrak{C}$ and let $\lambda_t(z) = Z(z; \cdot, \cdot)_\#\gamma$. Then we have that the radial conditional law of $\lambda_t$ is deterministic.
\end{restatable}
\noindent \textit{Proof.} Under the hypotheses listed, we know that $\lambda_0$ and $\lambda_1$ have a deterministic conditional radial law and their optimal coupling is supported on a map (\Cref{clancy coupling supported on a map}) given by
\[(x_0, r_0(x_0)) \mapsto (T(x_0), r_1(T(x_0)).\]
...
where $\lambda_{0, x}$ is the $x$-marginal of $\lambda_0$. It suffices to show that the map $X(t; [x_0, r_0(x_0)], [T(x_0), r_1(T(x_0))])$ (\Cref{eq: cone geodesics}) is injective for all $t$ -- we can do this by showing that $x \mapsto X_t(x)$ is \textit{strongly monotone} on $\Omega$ (\citet{facchinei2003finite}, Proposition 2.3.2). We have
\begin{align*}
    X(t; z_0, z_1) = (1 - \rho(t; z_0, z_1))x_0 + \rho(t; z_0, z_1)T(x_0)
\end{align*}
The Jacobian is given by
\begin{align*}
    \nabla_{x_0}X(t) &= I_d + \nabla \rho(t)(T(x_0) - x_0)^T + \rho(t)(\nabla T(x_0) - I_d) \\
    &= (1 - \rho_t(x))I_d + \rho_t(x)\nabla T(x) + (T(x) - x)\nabla\rho_t(x)^T
\end{align*}
and define $\text{Sym}(A) := \frac{1}{2}(A + A^T)$. Then
\begin{align*}
    \text{Sym}(\nabla X(t)) = (1 - \rho_t(x))I_d + \rho_t(x)\cdot \text{Sym}(\nabla T) + \frac{1}{2}\left[(T(x) - x)\nabla \rho_t(x)^T + \nabla \rho_t(x)(T(x) - x)^T\right]
\end{align*}
Note that the last term has operator norm bounded by $\|T(x) - x\|_2\|\nabla\rho_t(x)\|_2$. Thus, 
\[\lambda_{\min}(\text{Sym}(\nabla X(t))) \geq  (1 - \rho_t(x)(1 - m_T)) - \|T(x) - x\|_2 \|\nabla\rho_t(x)\|_2 \quad \text{for $\lambda_0$ a.e. } x \in \Omega. \tag{\textasteriskcentered}\]
For any $t$ in which the right-hand expression is positive, we have that $X(t; z_0, z_1)$ is injective. This injectivity ensures that the radial conditional determinism of $\lambda_0$ is preserved when pushed forward through $X_t$, guaranteeing that $\lambda_t$ has a radial conditional law that is deterministic too.
\qed{}
\\

% \begin{restatable}[Conditional Radial Law on Geodesic Interior]{lemma}{} \label{deterministic radial law}
%     Let $(\mu_t)_{t \in [0, 1]}$ be an absolutely continuous HK geodesic and let $\lambda_0, \lambda_1$ be the optimal lift of the endpoints obtained from \Cref{alg: let lift} where $\mu_i^\perp = 0$ and $\eta$ is supported on a map $T$ such that $\frac{1}{2}(\nabla T(x) + \nabla T(x)^T) \succeq m_T I_d$. Also let $\gamma \in \mathcal{P}_2(\mathfrak{C}_\Omega \times \mathfrak{C}_\Omega)$ be an optimal coupling in the sense of $W_\mathfrak{C}$ and let $\lambda_t(z) = Z(z; \cdot, \cdot)_\#\gamma$. Then there exists a $\varepsilon_0$ sufficiently small such that for all $\varepsilon \leq \varepsilon_0$, we have that the radial conditional law of $\lambda_\varepsilon, \lambda_{1-\varepsilon}$ is deterministic.
% \end{restatable}
% \noindent \textit{Proof.} Under the hypotheses listed, we know that $\lambda_0$ and $\lambda_1$ have a deterministic conditional radial law and their optimal coupling is supported on a map (\Cref{clancy coupling supported on a map}) given by
% \[(x_0, r_0(x_0)) \mapsto (T(x_0), r_1(T(x_0)).\]
% The interpolated measures $\lambda_t$ therefore can be expressed by 
% \[\lambda_t = (z_t)_\#\lambda_{0,x} \quad \text{where}\quad z_t(x_0) = Z_t([x_0, r_0(x_0)], [T(x_0), r_1(T(x_0))])\]
% where $\lambda_{0, x}$ is the $x$-marginal of $\lambda_0$. It suffices to show that the map $X(t; [x_0, r_0(x_0)], [T(x_0), r_1(T(x_0))])$ (\Cref{eq: cone geodesics}) is injective for $t \in \{\varepsilon, 1-\varepsilon\}$ for $\varepsilon$ sufficiently small -- we can do this by showing that $x \mapsto X_t(x)$ is \textit{strongly monotone} on $\Omega$ (\citet{facchinei2003finite}, Proposition 2.3.2). We have
% \begin{align*}
%     X(t; z_0, z_1) = (1 - \rho(t; z_0, z_1))x_0 + \rho(t; z_0, z_1)T(x_0)
% \end{align*}
% The Jacobian is given by
...
% \end{align*}
% Note that the last term has operator norm bounded by $\|T(x) - x\|_2\|\nabla\rho_t(x)\|_2$. Thus, 
% \[\lambda_{\min}(\text{Sym}(\nabla X(t))) \geq m_0 - \|T(x) - x\|_2 \|\nabla\rho_t(x)\|_2 \quad \forall \, x \in \Omega \tag{\textasteriskcentered}\]
% for $m_0 \triangleq \min\{1, m_T\}$. For any $t$ in which the right-hand expression is positive, we have that $X(t; z_0, z_1)$ is injective. This injectivity ensures that the radial conditional determinism of $\lambda_0$ is preserved when pushed forward through $X_t$, guaranteeing that $\lambda_t$ has a radial conditional law that is deterministic too. We can use our expression of $\rho$ to further expand the condition (\textasteriskcentered), 
% \begin{align*}
%     \rho(t; [x_0, r_0(x_0)], [T(x_0), r_0(T(x_0))]) = \frac{1}{\theta}\arccos\left(\frac{(1-t)r_0 + sr_1\cos\theta}{R(s; r_0, r_1, \theta)}\right)
% \end{align*}
% where $\theta = \|x_0 - T(x_0)\|_2$ and $r_1 = r_1(T(x_0))$. By 
% \Cref{rho expansion}, for small $t$ we can expand as
% \begin{align*}
%     \rho(t; z_0, z_1) = t\frac{r_0}{r_1}\frac{\sin\theta}{\theta} + O(t^2) \quad \text{and} \quad 1 - \rho(1-t; z_0, z_1) = t\frac{r_0}{r_1}\frac{\sin\theta}{\theta} + O(t^2).
% \end{align*}
% Thus, the partials satisfy $\partial_{r_0}\rho_t = O(t)$, $\partial_{r_1}\rho_t = O(t)$ and $\partial_\theta\rho_t = O(t)$ as $t \downarrow 0$. By chain rule, it follows that $\|\nabla\rho_\varepsilon\|_{L_\infty(\Omega)} \lesssim \varepsilon$ and $\|\nabla\rho_{1-\varepsilon}\|_{L_\infty(\Omega)} \lesssim \varepsilon$. This implies that for $\varepsilon$ sufficiently small, (\textasteriskcentered) is satisfied.
% \qed{}
% \\
...
This implies that $\nabla^\mathfrak{C}_{\dot \lambda_t}u_t = 0$, rendering it parallel along $\lambda_t$. Since $\mathcal{P}_{\lambda_t}[u_t] = \mathbf{u}_t$, the thesis of \Cref{commutator} is proven. \qed{}


\subsection{Proof of \Cref{parallel transport approx entire geodesic clipped}.} \label{sec: proof of parallel transport approx entire geodesic clipped}
Recall the explicit bound from \Cref{one step parallel transport approx result}
\begin{multline*}
    \|\widehat{\text{PT}}_k(v) - \text{PT}_k(v)\|_{L_2(\nu_{ks})} \leq As^2\|v\|_{L_2(\nu_{(k-1)s})}\\
    + \left(e^{\left(\int_\varepsilon^{1-\varepsilon}\text{L}(\nabla \psi_r)dr\right)}  - 1\right)^2\|v\|_{L_2(\nu_{(k-1)s})}\left(\int_{(k-1)s}^{ks}\text{L}(\nabla\psi_r)dr\right)^2.
\end{multline*}
...
\begin{restatable}{lemma}{} \label{rho expansion}
    We can employ the following small $t$ expansions of $\rho(t; z_0, z_1)$ and $\rho(1-t; z_0, z_1)$:
    \[\rho(t; z_0, z_1) = t\frac{r_0}{r_1}\frac{\sin\theta}{\theta} + O(t^2) \quad \text{and} \quad 1 - \rho(1-t; z_0, z_1) = t\frac{r_0}{r_1}\frac{\sin\theta}{\theta} + O(t^2)\]
    when $\theta < \pi/2$.
\end{restatable}
\noindent \textit{Proof.} We can rewrite $R(t; r_0, r_1, \theta)^2$ as
\begin{align*}
    R(t)^2 = r_0^2 + 2tr_0(r_1\cos \theta - r_0) + t^2(r_0^2 + r_1^2 -2r_0r_1\cos \theta).
\end{align*}
Define $n_1 \triangleq r_1\cos \theta - r_0$ and $d \triangleq r_0^2 + r_1^2  - 2r_0r_1\cos\theta$. It then holds that
\[R(t) = r_0\sqrt{1 + \frac{2tn_1}{r_0} + \frac{t^2d}{r_0^2}} = r_0 + tn_1 + t^2\frac{d - n_1^2}{2r_0} + O(t^3)\]
using the fact that $\sqrt{1 + u} = 1 + \frac{1}{2}u - \frac{1}{8}u^2 + O(u^2)$. Noting that $d - n_1^2$ can be simplified to $r_1^2\sin^2\theta$ implies
\[R(t) = r_0 + tn_1 + t^2\frac{r_1^2\sin^2\theta}{2r_0} + O(t^3).\]
Now let $N(t) \triangleq (1-t)r_0 + tr_1\cos\theta = r_0 + tn_1.$ Then
\[\frac{N(t)}{R(t)} = \frac{r_0 + tn_1}{r_0 + tn_1 + t^2\frac{r_1^2\sin^2\theta}{2r_0} + O(t^3)} = 1 - \frac{t^2r_1^2\sin^2\theta}{r_0^2} + O(t^3).\]
Now we will use the following expansion of $\arccos$ about $1$, 
\[\arccos(1 - u) = \sqrt{2u} + O(u^{3/2})\]
with $u \triangleq 1 - \frac{N(t)}{R(t)} = \frac{t^2}{2}(\frac{r_1}{r_0})^2\sin^2\theta + O(s^3) = O(s^2). $ Plugging in and dividing by $\theta$, we get
\[\rho(t) = t\frac{r_1}{r_0}\frac{\sin \theta}{\theta} + O(t^2).\]
By symmetry of the geodesic interpolant in \Cref{eq: cone geodesics}, we have that $\rho(t; z_0, z_1) = 1 -\rho(t; z_1, z_0).$ It follows that 
\[1 - \rho(1-t; z_0, z_1) = t\frac{r_0}{r_1}\frac{\sin \theta}{\theta} + O(t^2)\]
for small $t$. 
\qed{}
\\
...
\end{align*}
Since for all $\phi \in C^0(\Omega)$, 
\[\int_\Omega \phi(x)d\mathfrak{B}\lambda = \int_{\mathfrak{C}_\Omega}r^2\phi(x)d\lambda(x,r)\]
we can chose $\phi \equiv 1$ to say
\begin{align*}
     W_\mathfrak{C}(\lambda_0, \lambda_1) &\leq \left(\int_{\mathfrak{C}_\Omega}r^2 \, d\lambda_0(x,r)\right)^{1/2} + \left(\int_{\mathfrak{C}_\Omega}r^2 \, d\lambda_1(x,r)\right)^{1/2} \\
     &= \left(\int_{\Omega} \, d\mu_0(x)\right)^{1/2} + \left(\int_{\Omega} \, d\mu_1(x)\right)^{1/2} \\
